...
const TOC = [
  { id: "california", label: `${CA_RACE_COUNT} California Races` },
  { id: "us-races", label: `${US_RACE_COUNT} US Races` },
...
// The "as of" date the FAQ and the download gate both quote. Derived from the
// OLDEST `checked` stamp in the guide, not the newest and not a hand-typed
// month: the only honest reading of "status updated X" is that every race in
// the list has been confirmed since X. Taking the newest stamp would let one
// race verified today paper over twenty verified in July.
//
// Practical effect: this date only moves forward when the stalest race gets
// re-verified, so a guide that looks fresh actually is. The daily freshness
// agent works oldest-stamp-first for exactly this reason.
const VERIFIED = (() => {
  const stamps = [...CA_RACES, ...US_RACES]
    .map(r => r.checked)
    .filter((c): c is string => Boolean(c))
    .sort();
  if (stamps.length < CA_RACES.length + US_RACES.length || stamps.length === 0) {
    // An unstamped race means we cannot vouch for the whole list.
    return "an unverified date";
  }
...
function caStatusAnswer(): string {
  ...
  if (sold.length > 0) {
    // Says "check charity options" no longer: Monterey Bay's benefactor entries
    // sold out in fifteen minutes while this sentence was still sending readers
    // to look for them. Each row carries its own way-in, so point at that.
    parts.push(
      `Sold out at standard entry: ${listSentence(sold.map(r => `${r.name} (${label(r)})`))}. ` +
      "Each row says whether a waitlist or charity entry is still open."
    );
  }

  // A race that has been run is not "open", whatever its last registration
  // status said. Without this bucket a past race kept its old status and the
  // answer went on advertising it.
  const past = season.filter(r => r.status === "past");
  if (past.length > 0) {
    parts.push(`Already run this season: ${listSentence(past.map(r => r.name))}.`);
  }
...
export default function OpenEntryRaces2026() {
  return (
    <>
      <ArticleJsonLd {...META} datePublished="2026-06-14" dateModified="2026-08-21" />
      <FaqJsonLd faqs={FAQS} />
      {/* NAV */}
      <SiteNav />
      ...
      <main className="post">
        ...
        <section className="article-masthead">
          <div className="page">
            ...
            <h1 className="article-headline">
              {TOTAL_RACE_COUNT} Open Entry Races in <span>California</span> and the US You Can Still Run in 2026
            </h1>
          </div>
        </section>
        ...
        <section className="article-hero">
          <div className="page">
            <p className="article-deck">
              No qualifying time. No lottery. {CA_RACE_COUNT} California races, {US_RACE_COUNT} across the US.
        ...
        <section className="article-body">
          <div className="page">
            <p>
              ...
              something, this is it. {TOTAL_RACE_COUNT} open entry road races: {CA_RACE_COUNT} in California, {US_RACE_COUNT} across the US.
        ...
        <section id="california" style={{ borderBottom: "1px solid var(--line)", paddingBottom: "56px" }}>
          <div className="page">
            <div className="article-section-head">
              <div className="article-section-label">{CA_RACE_COUNT} California Races</div>
              <div className="article-section-sub">Open Entry · Summer 2026 to Spring 2027</div>
            </div>
            <div className="race-list">
              {CA_RACES.map((r) => <RaceRow key={r.num} race={r} registerLabel="Register →" />)}
            </div>
          </div>
        </section>
        ...
        <section id="us-races" style={{ borderBottom: "1px solid var(--line)", paddingBottom: "56px" }}>
          <div className="page">
            ...
            <div className="race-list">
              {US_RACES.map((r) => <RaceRow key={r.num} race={r} registerLabel="Register →" />)}
            </div>
          </div>
        </section>
